...
<div class="container mt-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['insertar'])) {
        // Recogemos los tres datos del formulario
        $dni = $_POST['dni'];
        $legajo = $_POST['legajo'];
        $curso = $_POST['curso'];
        ...
        if (empty($dni) || empty($legajo) || empty($curso)) {
            ...
            // Insertamos los tres valores en la base de datos
    ...
    if ($resultado->num_rows > 0) {
        ...
        while($fila = $resultado->fetch_assoc()) {
            echo '<tr>';
            echo '<td class="table-primary">' . $fila["pk_alumno"] . '</td>';
            echo '<td class="table-secondary">' . $fila["n_legajo"] . '</td>';
            echo '<td class="table-success">' . $fila["dni"] . '</td>';
            echo '<td class="table-warning">' . $fila["curso"] . '</td>';
            echo '</tr>';
        }
